...
def gridsearchLGBM(data, cv=5, size='s', debug=False):
    ...
    list_reg_alpha = [0.0, 20]
    list_reg_lambda = [0.0, 50]
...
def gridsearchLGBM2(data, cv=3, debug=False):
    y = data.iloc[:, 0]
    X = data.iloc[:, 1:]
    ...
    lgbm = LGBMClassifier(num_leaves=20, max_bin=63, min_child_samples=20, n_jobs=1)
    clf = GridSearchCV(lgbm, parameters, scoring='roc_auc', n_jobs=1, cv=cv, return_train_score=True)
    clf.fit(X,y)
    ...
    if debug:        
        for i in range(len(clf.cv_results_.get('mean_train_score'))):
            print(clf.cv_results_.get('params')[i], '\n', f'{clf.cv_results_.get("mean_train_score")[i]:.4f}', f'{clf.cv_results_.get("mean_test_score")[i]:.4f}', '\n')
